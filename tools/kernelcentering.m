...
%function Kc=kernelcentering(K,Ktrain)
%
...
%K = K - S'*ones(1,Nj) - (ones(Ni,1)/Ni)*sum(K) + (sum(S)/Ni)*ones(Ni,Nj); 
K(isnan(K))=0; K(isinf(K))=0;
...
Ktest(isnan(Ktest))=0; Ktest(isinf(Ktest))=0;
